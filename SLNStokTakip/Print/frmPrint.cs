...
namespace SLNStokTakip.Print
{
    public partial class frmPrint : Form
    {
        ...
        private void FaturaList()
        {
            frmFaturaKesListe kl = Application.OpenForms["frmFaturaKesListe"] as frmFaturaKesListe;

            frmFaturaListesi ft = new frmFaturaListesi();

            var lst = (from s in _db.vwFaturaKes
                       select s).ToList();
            if (lst != null)
            {
                PrintYardım ch = new PrintYardım();
